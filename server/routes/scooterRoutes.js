...
router.put('/:scooter_id/update-hire', async (req, res) => {
    const scooterId = req.params.scooter_id;
    const { scooter_hire } = req.body;

    const connection = await pool.getConnection();
    try {
        const result = await connection.query(
            'UPDATE scooters SET scooter_hire = ? WHERE scooter_id = ?',
            [scooter_hire, scooterId]
        );

        if (result.affectedRows > 0) {
            res.json({ message: 'Scooter hire status updated successfully' });
        } else {
            res.status(404).json({ error: 'Scooter not found' });
        }
    } catch (error) {
        console.error('Error updating scooter hire status:', error.message);
        res.status(500).json({ error: 'Error updating scooter hire status', details: error.message });
    } finally {
        connection.release();
    }
});
